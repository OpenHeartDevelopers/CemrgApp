...
void FestiveView::OnSelectionChanged(
...
void FestiveView::CopyServer() {
    ...
    int stat0 = std::system(("rm -rf " + (directory + dirSep + "Transfer").toStdString()).c_str());
    int stat1 = std::system(("mkdir "  + (directory + dirSep + "Transfer").toStdString()).c_str());
    if (stat0 < 0 || stat1 < 0)
        QMessageBox::warning(NULL, "Attention", "File operations failed!");
    imgsList.clear();
