...
const std::string WallThicknessCalculationsClipperView::VIEW_ID = "my.cemrgproject.views.WallThicknessCalculationsClipperView";
...
void WallThicknessCalculationsClipperView::iniPreSurf() {
    ...
    if (data) {
        ...
        if (image) {
            ...
            //Check seg node name
            if (segNode->GetName().compare(fileName.left(fileName.indexOf('.')).toStdString()) != 0) {
            ...
            if (dialogCode == QDialog::Accepted) {
                ...
                deci->SetTargetReduction(static_cast<double>(ds));
                ...
                computeNormals->SetFeatureAngle(360.0);
                ...
                //Figure flipping lines
                try {
                    double centre[3];
                    vtkSmartPointer<vtkCenterOfMass> centerOfMassFilter = vtkSmartPointer<vtkCenterOfMass>::New();
                    centerOfMassFilter->SetInputData(shell->GetVtkPolyData());
                    centerOfMassFilter->SetUseScalarsAsWeights(false);
                    centerOfMassFilter->Update();
                    centerOfMassFilter->GetCenter(centre);
                    vtkSmartPointer<vtkPointLocator> pointLocator = vtkSmartPointer<vtkPointLocator>::New();
                    pointLocator->SetDataSet(shell->GetVtkPolyData());
                    pointLocator->BuildLocator();
                    vtkIdType id = pointLocator->FindClosestPoint(centre);
                    double* cPoint = shell->GetVtkPolyData()->GetPoint(id);
                    centre[0] = centre[0] - cPoint[0];
                    centre[1] = centre[1] - cPoint[1];
                    centre[2] = centre[2] - cPoint[2];
                    vtkFloatArray* normals = vtkFloatArray::SafeDownCast(shell->GetVtkPolyData()->GetPointData()->GetNormals());
                    double* normal = normals->GetTuple(id);
                    double product = 0;
                    for (int i = 0; i <= 2; i++)
                        product += (centre[i])*(normal[i]);
                    m_Controls.checkBox->setChecked(product<0?false:true);
                } catch (...) {}
                this->BusyCursorOff();
