...
void CemrgAtriaClipper::ClipVeinsImage(std::vector<int> pickedSeedLabels, mitk::Image::Pointer segImage, bool morphAnalysis) {
    ...
    for (unsigned int i=0; i<pickedSeedLabels.size(); i++) {
        ...
        if (morphAnalysis) {
            ...
            double diMn = diams->GetValue(position);
            diams = vtkDoubleArray::SafeDownCast(line->GetPointData()->GetArray("CenterlineSectionMaxSizeArrayName"));
            double diMx = diams->GetValue(position);
            ...
            morphResult << pickedSeedLabels.at(i) << " " << diMn << "\n";
            morphResult << pickedSeedLabels.at(i) << " " << diMx << "\n";
            morphResult.close();
        }//_if
