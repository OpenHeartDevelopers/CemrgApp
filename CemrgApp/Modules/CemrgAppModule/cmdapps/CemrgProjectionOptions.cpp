...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (fopts.exists() && fopts.isFile()) {
            ...
            // measurement_type 
            if (json.contains("measurement_type")){
                measurement = json["measurement_type"].toInt();
                if (verbose) std::cout << "JSON set measurement type to " << measurement << '\n';
            }
            // thresholds_method
        ...
        // check that measurement is not out of bounds 
        ...
        int minStep = small;
        int maxStep = big;
        int measureType = measurement; // Mean=1, Max=2, Cumulative=3, Mode=4
