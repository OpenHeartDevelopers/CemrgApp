...
mitk::Surface::Pointer CemrgPower::MapPowerTransmitterToLandmarks(mitk::DataNode::Pointer lmNode) {
    ...
    */
    ...
    if (mps.size() >= 3) {
        ...
        vw_cas[0]=(mps[2][0]-mps[0][0]);
        vw_cas[1]=(mps[2][1]-mps[0][1]);
        vw_cas[2]=(mps[2][2]-mps[0][2]);
    ...
    vtkSmartPointer<vtkMatrix4x4> vtk_T =
            vtkSmartPointer<vtkMatrix4x4>::New();
    vtkSmartPointer<vtkMatrix4x4> T0 =
            vtkSmartPointer<vtkMatrix4x4>::New();
    vtkSmartPointer<vtkMatrix4x4> T1 =
            vtkSmartPointer<vtkMatrix4x4>::New();
    vtkSmartPointer<vtkMatrix4x4> T2 =
            vtkSmartPointer<vtkMatrix4x4>::New();
    vtkSmartPointer<vtkMatrix4x4> T3 =
            vtkSmartPointer<vtkMatrix4x4>::New();
...
mitk::Surface::Pointer CemrgPower::CalculateAcousticIntensity(mitk::Surface::Pointer endoMesh) {
    ...
    ******************************************************************/
    ...
    double rx[3];
    float tempx, tempy, tempI;
    int size_ids=0;
    // Calculate the power intensity for each point
    for (int i=0; i<endo_polydata->GetNumberOfPoints(); i++) {
        //for (int i=0; i<2; i++) {
        //endo_polydata ->GetPoint(i, rx);
        endo_pointSet->GetPoint(i, rx);

        d = sqrt(pow(rx[0],2.0)+pow(rx[1],2.0)+pow(rx[2],2.0)); // magnitude of r vector
        tempx = (M_PI*L*rx[0]) / (l*d);
        tempy = (M_PI*L*rx[1]) / (l*d);
        tempI=( A / (pow(l,2.0) * pow(d,2.0)) ) * pow(10.0,(d*atten*f)/1E5) * sinc(tempx)*sinc(tempy);
...
void CemrgPower::fcn_RotationFromTwoVectors(
        double a[], double b[], vtkSmartPointer<vtkMatrix3x3>& RotationMatrix) {
    ...
    // Check if dot product > 1

    if (CemrgPower::dotProduct(a,b)>1.0) {
        angle=1.0;
    } else {

        angle=acos(CemrgPower::dotProduct(a,b));
        s=sin(angle);
        c=cos(angle);
        t=1-c;
        //cerr << "t= " << t << endl;
