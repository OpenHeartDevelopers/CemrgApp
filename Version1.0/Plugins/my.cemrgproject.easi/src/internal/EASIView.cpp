...
const std::string EASIView::VIEW_ID = "my.cemrgproject.views.easiview";
...
void EASIView::Simulation() {
    ...
    //curvesCalculator();
    //testMonoContract();
    //AddToStorage("TEST",testDICOMtagOFinterest(this->GetDataManagerSelection())->GetData());
    //create4DImagesfromDICOM();
    //calcMorphAuto();
    //cleanAutoSegResults();
    //createMVImagesVTK();
    //convertfromDICOM();
    //downsampleImages();
    //autoMeshDeformer();
    //autoScarCalculations();
    //autoScarCalculationsBasedonShape();
    //generateLocationMap();
    //singleCurvesCalculator();
    //generateAtlas();
    //convertVTKtoMPS();
    //calcSphericity();
    //autoNIIconvert();
    newCTveinClipperBasedonDistance();
}
