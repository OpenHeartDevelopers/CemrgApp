...
std::vector<double> CemrgStrains::CalculateSqzPlot(int meshNo) {
    ...
    vtkSmartPointer<vtkFloatArray> ahaValues = vtkSmartPointer<vtkFloatArray>::New();
    ...
    for (vtkIdType cellID = 0; cellID < pd->GetNumberOfCells(); cellID++) {
        ...
        ahaValues->InsertNextTuple1(refCellLabels[cellID]);

        index++;
    }//_for

    //Store Squeeze and AHA values
    sqzValues->SetName("Squeez");
    pd->GetCellData()->AddArray(sqzValues);
    ahaValues->SetName("AHA");
    pd->GetCellData()->AddArray(ahaValues);
...
mitk::Surface::Pointer CemrgStrains::ReadVTKMesh(int meshNo) {
    ...
    mitk::Surface::Pointer surf = mitk::IOUtil::LoadSurface(meshPath.toStdString());
    vtkSmartPointer<vtkPolyData> pd = surf->GetVtkPolyData();
